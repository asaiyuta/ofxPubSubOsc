...
#define instantiate_parameters(...) \
            extern template struct Parameter<__VA_ARGS__>;\
            extern template auto Subscriber::make_parameter_ref(__VA_ARGS__ &value) -> enable_if_t<!is_callable<__VA_ARGS__>::value, ParameterRef>;\
            extern template SubscribeIdentifier Subscriber::subscribe<__VA_ARGS__ &>(const std::string &, __VA_ARGS__ &);\
            extern template struct SetterFunctionParameter<void, __VA_ARGS__>;\
            extern template ParameterRef Subscriber::make_parameter_ref(std::function<void(__VA_ARGS__)>);\
            extern template SubscribeIdentifier Subscriber::subscribe<const std::function<void(__VA_ARGS__)> &>(const std::string &, const std::function<void(__VA_ARGS__)> &);\
            extern template SubscribeIdentifier Subscriber::subscribe< std::function<void(__VA_ARGS__)> &&>(const std::string &, std::function<void(__VA_ARGS__)> &&);
...
#define instantiate_parameters(RET, ...) \
            extern template struct SetterFunctionParameter<RET, __VA_ARGS__>;\
            extern template ParameterRef Subscriber::make_parameter_ref(std::function<RET(__VA_ARGS__)>);\
            extern template SubscribeIdentifier Subscriber::subscribe<const std::function<RET(__VA_ARGS__)> &>(const std::string &, const std::function<RET(__VA_ARGS__)> &);\
            extern template SubscribeIdentifier Subscriber::subscribe< std::function<RET(__VA_ARGS__)> &&>(const std::string &, std::function<RET(__VA_ARGS__)> &&);
